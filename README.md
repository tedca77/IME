...
IME will discover photos on your storage drives, and identify duplicate photos in different folders. IME has been designed so that it can be run repeatedly on a collection of photos – it will not attempt to redo geocoding if it has already been processed. It can also copy files to a new, organised structure based on Year and Month, leaving behind any duplicate files.
...
* **IPTC** – this contains the additional geographical information e.g. City, State /Province, Country etc.  Windows Explorer and Windows Properties do not show most of the IPTC data – the easiest way to see this is using the freeware IrfanView,( an excellent image viewer and editor which is useful for sizing and cropping photos).  ExifTool is another excellent program that can be used to view all metadata.  You can also use Adobe Lightroom (which I also use) and other professional software most of which is quite expensive. Adobe Bridge is free software which provides equivalent to Lightroom for editing IPTC metadata.
...
*	Every photo taken on a birthday could be given a title of the person's birthday
...
Events are defined in the JSON file – if the date provided in the JSON file matches the photo creation date (**EXIFOriginalDate**), then the event information will be copied across to the photo – this can also include geographical information on the event, if it exists.  
